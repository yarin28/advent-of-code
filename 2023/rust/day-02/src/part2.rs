...
pub fn process(input: &str) -> miette::Result<String, AocError> {
    ...
    let result: usize = games
        .iter()
        .map(|game| {
            let binding: Vec<SumOfColorsInElfGrab> = game
                .subset_of_cubes
                .iter()
                ...
                .collect();
            let res = SumOfColorsInElfGrab {
                red: binding.iter().map(|elf_grab| elf_grab.red).max().unwrap(),
                green: binding.iter().map(|elf_grab| elf_grab.green).max().unwrap(),
                blue: binding.iter().map(|elf_grab| elf_grab.blue).max().unwrap(),
            };
            res
        })
        .map(|game_sum| game_sum.red * game_sum.blue * game_sum.green)
        .sum();
    Ok(result.to_string())
}
