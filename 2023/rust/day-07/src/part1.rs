...
impl Hand {
    pub fn evalueate_hand_type(&mut self) {
        ...
        let max_value = card_symbol_and_amount.values().max().unwrap();
        if max_value == &5 {
            self.hand_type = Some(Type::FiveOfAKind);
        } else if max_value == &4 {
            self.hand_type = Some(Type::FourOfAKind);
        } else if max_value == &3 && card_symbol_and_amount.values().any(|num| *num == 2) {
            self.hand_type = Some(Type::FullHouse);
        } else if max_value == &3 && card_symbol_and_amount.values().all(|num| *num != 2) {
...
fn create_card_type_map() -> BTreeMap<char, i32> {
    ...
    card_type.insert('A', 13);
    card_type.insert('K', 12);
    card_type.insert('Q', 11);
    card_type.insert('J', 10);
    card_type.insert('T', 9);
    card_type.insert('9', 8);
    card_type.insert('8', 7);
    card_type.insert('7', 6);
    card_type.insert('6', 5);
    card_type.insert('5', 4);
    card_type.insert('4', 3);
    card_type.insert('3', 2);
    card_type.insert('2', 1);
    card_type
}
